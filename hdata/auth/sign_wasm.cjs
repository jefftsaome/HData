 ...
 *
 * 【真源位置】本文件随 hdata 包分发（hdata/auth/），是签名逻辑的唯一实现。
 * 项目根 scripts/sign_wasm.cjs 只是转发入口，修改签名逻辑请改本文件。
 *
 * 用法:
 *   node hdata/auth/sign_wasm.cjs <path前缀> [env]
 *   node hdata/auth/sign_wasm.cjs /site/api prod
 *
