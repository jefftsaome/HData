...
> **【2026-07-25 加注：本文为早期（hdt 时代）过程性研究笔记，多处结论已被取代】**
> - §1 AES IV/Key 拆分说法已废弃：现行实现 key=16 字节随机 hex、
>   IV=ASCII `'0'×16`（《极验4验证码破解与排查.md》§2.1）；
> - §5「verify 始终 fail」/§7「e_obj 字段差异未解决」：已于 2026-07-17
>   解决（字段集与浏览器运行时对齐，纯 HTTP 全链路通过）；
> - §8 e_obj 字段中的 `biht`/`gee_guard` 为旧版字段，浏览器已停发。
> **现行权威文档：《极验4验证码破解与排查.md》；以下为历史记录。**
