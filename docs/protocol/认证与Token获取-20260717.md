...
> 结论截至:2026-07-17(纯 HTTP 全链路打通,平台更新后可能失效)
## 整体认证流程
...
WebSocket 连接（2026-07-17 已打通，见 协议报文目录-20260627.md §9）:
...
1. **X-API-XXX 动态签名**：算法在 wasm（`wasm_api_sign`）中，`sign(path前缀,"prod")` 每请求唯一。
   ...
   服务端对 /game/api、login 等强制校验，假签名返回 6003。详见 `docs/登录接口抓包实测-20260717.md`。
...
3. **GeeTest w / e_obj**：hook Math.random 爆破 AES 密钥解密真实 e_obj，修正字段集
   ...
   详见 `docs/验证码验证流程-20260717.md` 附录。
