...
**w 参数加密（实测确认 1216 hex，与 验证码验证流程-20260717.md 最新结论一致；旧文档的 1568 已过时）：**
```
...
**旧假设（认证与Token获取-20260717.md）：签名是按 path 固定的静态表。实测不成立。**
...
e_obj 字段集/顺序已与浏览器运行时对齐（见《极验4验证码破解与排查-20260718.md》§2.3），
